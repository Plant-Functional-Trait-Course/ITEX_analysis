...
standardize_fluxes <- function(CommResp, Height, ITEX.data.post.calcs){
...
flux_and_trait <- function(Trait_Mean, Flux_and_Traits){

  ITEX.Trait_Fluxes <- Trait_Mean %>%
    rename(ITV = mean,
           No_ITV = mean_noitv) %>%
    ...
    left_join(Flux_and_Traits, by = c("Site", "PlotID", "Treatment"))
...
trait_model_selelction <- function(Flux_and_Traits){

  ITEX.mean.fluxes <- Flux_and_Traits %>%
    rename(CWM_C = ITV_C,
           CWM_CN = ITV_CN,
           CWM_LDMC = ITV_LDMC,
           CWM_LA = ITV_LA,
...
model_selection <- function(Flux_and_Traits){
...
model_selection_results <- function(Flux_and_Traits){
